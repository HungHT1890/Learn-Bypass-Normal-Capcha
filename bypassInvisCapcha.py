...
def bypass_capcha(url_get,url_post,proxy=None):
    try:
        ss = session()
        recaptcha_token = ss.get(url=url_get,proxies= proxy , timeout = 20,verify=False).text.split('<input type="hidden" id="recaptcha-token" value="')[1].split('">')[0]
        ...
        capcha_solver = ss.post(url=url_post,headers=header,data=post_data , proxies= proxy , timeout = 20,verify=False).text.split('["rresp","')[1].split('",')[0]
